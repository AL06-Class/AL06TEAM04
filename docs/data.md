...
```text
- 신입 지원 가능 또는 experienceMin = 0이면 entry를 사용합니다.
- 필수 최소 경력이 없거나 경력 우대만 있으면 any를 사용합니다.
- experienceMin이 1~2년이면 junior, 3~4년이면 mid, 5년 이상이면 senior를 사용합니다.
- 원문의 세부 경력 범위는 experienceMin과 experienceMax에 보존합니다.
```
...
- 2026-07-15: 외부 공고 경력 수준을 최소 경력 기준의 공통 개발 값으로 정규화
